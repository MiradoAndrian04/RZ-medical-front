...
const AccountSettings = () => {
    ...
    const fetchUser = async () => {
      try {
        ...
        const response = await fetch(
          `${import.meta.env.VITE_APP_API_URL}/admin/utilisateur`,
          {
            method: "GET",
            headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    const isValidPhoneNumbers = formData.telephones.every(
      (phone) => phone.length === 10
    );

    if (!isValidPhoneNumbers) {
      toast.error(
        "Chaque numéro de téléphone doit contenir exactement 10 chiffres."
      );
      return;
    }
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_APP_API_URL}/admin/update/${user.id}`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleValidationSubmit = async (e) => {
    ...
    try {
      const token = JSON.parse(localStorage.getItem("admin-user"));
      const response = await fetch(
        `${import.meta.env.VITE_APP_API_URL}/validation`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
