...
const ShowProduit = () => {
    ...
    const fetchProduct = async () => {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_APP_API_URL}/produit/${id}`
        );
        if (!response.ok)
        ...
        const categoryResponse = await fetch(
          `${import.meta.env.VITE_APP_API_URL}/categorie/${
            data.produit.categorie_id
          }`
        );
  ...
  const confirmDelete = async () => {
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_APP_API_URL}/admin/produit/${id}`,
        {
          method: "DELETE",
          headers: {
      ...
      setProducts((prevProducts) =>
        prevProducts.filter((p) => p.id !== parseInt(id))
      );
      navigate("/products");
    } catch (error) {
  ...
  if (!product) {
    return (
      <div className="text-center text-gray-600">Produit introuvable.</div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white shadow-md rounded-lg p-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Image */}
        <img
          src={`${import.meta.env.VITE_APP_API_URL}/storage/${
        ...
        <div className="flex flex-col justify-between">
          <div>
            ...
            <h1 className="text-lg font-bold text-gray-808">
              {product.nom_produit}
            </h1>
            <p className="text-gray italic mt-[-5px] text-sm">{categoryName}</p>
            <p className="text-lg font-semibold text-green-600 my-3">
              {product.prix != 0 ? product.prix + " Ar" : "Prix à consulter"}
            </p>
            <hr />
            {product.description_produit &&
              product.description_produit.length > 15 && (
                <>
                  <h2 className="text-lg font-semibold text-gray-700 mt-3">
                    Description :
                  </h2>
                  <pre className="text-gray-600 whitespace-pre-wrap break-words overflow-x-auto max-w-full p-2">
                    {product.description_produit}
                  </pre>
                </>
              )}
